...
const EventMerchandise = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setIsLoading(true);
        const result = await api.get('/products?category=event-merchandise');
        if (result.success) {
    ...
    <div className="product-page-container">
      <section className="products-section" style={{ paddingTop: '40px' }}>
        <div className="container">
          <h2 style={{ fontSize: '32px', fontWeight: '800', marginBottom: '32px', color: '#0f172a' }}>Event Merchandise</h2>
